...
from .connection import rMConnect, UnknownHostKeyException
...
class rMViewApp(QApplication):
    ...
    def ensureConnConfig(self):
        ...
        if (auth_method == 'password' and 'password' not in self.config['ssh']) or \
            ...
            password, ok = QInputDialog.getText(self.viewer, "Configuration", "reMarkable password:",
                                                QLineEdit.Password)
            if ok:
    ...
    def connected(self, ssh):
        ...
        if version is None or version[1] not in ["1", "2"]:
            ...
            QMessageBox.critical(None, "Unsupported device",
        ...
        if out.channel.recv_exit_status() != 0:
            mbox = QMessageBox(QMessageBox.NoIcon, 'Missing components',
    ...
    def movePen(self, x, y):
        ...
        x = scaling * (x - (stylus_width - WIDTH / scaling) / 2)
        y = scaling * (y - (stylus_height - HEIGHT / scaling) / 2)
        if self.trail is not None:
